...
const GetAllevent = ({setShowCreate}) => {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter(
      (job) =>
        ...
        job?.venue?.toLowerCase().includes(searchText)
    );
  }, [jobs, search]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await axios.delete(`${base_url}/event/delete/${id}`, {
        withCredentials: true,
      });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Manage all your event postings
              </p>
            </div>
          </div>
        </div>
...
<th className="px-5 py-4 text-left font-semibold">
     ...
     <td>
<img src={`${img_url}${job.image}`} alt=""  className="h-24 "/>
